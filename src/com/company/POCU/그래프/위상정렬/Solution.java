package com.company.POCU.그래프.위상정렬;
...
class Solution {
    ...
    private static void topologicalSortRecursive(Course course, HashSet<Course> discovered, LinkedList<Course> sortedList){
        discovered.add(course);

        for (Course nextCourse : course.getNextCourses()) {

            if(discovered.contains(nextCourse)) continue;
            topologicalSortRecursive(nextCourse, discovered, sortedList);
        }

        sortedList.addFirst(course);
    }
